...
const NAV_ITEMS = [
  ...
  },
  {
    label: 'Keuangan',
    href: '/keuangan',
    match: (p: string) => p === '/keuangan' || p === '/keuangan/input',
  ...
  },
  {
    label: 'Produk / Menu',
    href: '/dashboard/menu',
    match: (p: string) => p.startsWith('/dashboard/menu'),
  ...
  },
  {
    label: 'Inventori',
    href: '/inventori',
    match: (p: string) => p.startsWith('/inventori'),
    icon: <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" /></svg>,
  },
  {
    label: 'Pelanggan',
    href: '/pelanggan',
    match: (p: string) => p.startsWith('/pelanggan'),
    icon: <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>,
  },
  {
    label: 'AI Analyst',
    href: '/ai-analyst',
    match: (p: string) => p === '/ai-analyst',
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="h-screen bg-[#121212] flex overflow-hidden text-white font-sans">

      {/* Sidebar */}
      <aside className={`flex-shrink-0 bg-black/40 backdrop-blur-md border-r border-white/10 transition-all duration-300 ${isSidebarOpen ? 'w-64' : 'w-20'} flex flex-col h-full`}>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="flex-shrink-0 h-20 flex items-center justify-between px-8 bg-black/20 backdrop-blur-sm border-b border-white/5">
          <h1 className="text-2xl font-semibold capitalize">
            {pathname === '/dashboard' ? 'Dashboard Utama' : pathname.split('/').pop()?.replace(/-/g, ' ')}
          </h1>
          <div className="flex items-center gap-4">
            <div className="text-right hidden md:block">
              <p className="text-sm font-medium">Cleco Pii / Admin</p>
            </div>
